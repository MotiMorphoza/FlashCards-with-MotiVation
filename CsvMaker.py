...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(root, text="Filename (no .csv)").grid(
        ...
        # ---- content ----
        self.content_box = ttk.LabelFrame(
            root, text="CSV Content per Language"
        )
        self.content_box.grid(
            row=3, column=0, columnspan=5, sticky="nsew", pady=10
        )
    ...
    def sync_from_disk(self):
        ...
        self.branch_cb["values"] = [
            b["id"] for b in self.index["branches"]
        ]
        self.update_groups()
    ...
    def generate(self):
        ...
        if not branch or not group or not filename:
            messagebox.showerror(
                "Error", "Branch / Group / Filename required"
            )
            return

        selected_langs = [
            lid for lid, v in self.lang_vars.items() if v.get()
        ]
        ...
        for lid in selected_langs:
            content = self.text_widgets[lid].get(
                "1.0", "end"
            ).strip()
            if not content:
                continue
